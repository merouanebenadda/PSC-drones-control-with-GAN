...
def generate_wave(n_samples) :
    ...
    k = 2*m.pi/0.5
    x = torch.linspace(-1/3, 1/3, n_samples, device=device)
...
def f_formation(sample_x, sample_x_pushforwarded, initial_positions_pushforwarded):
    ...
    x = torch.matmul(sample_x_pushforwarded, R.T)
    x_centered = x - x.mean(dim=0, keepdim=True)
    sigma = np.sqrt(variance)
    def density_estimated(pts):
        pts = pts.to(device)
...
def generate_sample(batch_size):
    return torch.rand((batch_size, 3), device=device)*2. - 1  # x dans [-0.5, 0.5]
...
if len(sys.argv) < 2:
    print("Le programme prend au moins un argument : indiquer 'train' si le modèle doit être entrainé, seulement 'load' sinon")
    print("Pour un deuxième argument, indiquer le nom du modèle (optionnel)")
    exit(1)

TRAIN = (sys.argv[1] in ("train", "t"))

PATH = pathlib.Path(sys.argv[0]).resolve().parent
MODEL_NAME = "model_" + ("default" if len(sys.argv) < 3 else sys.argv[2])
...
def main():
    ...
    try:
        N_omega.load_state_dict(torch.load(PATH_MODEL_N_OMEGA, weights_only=True))
    except:
    ...
    try:
        N_theta.load_state_dict(torch.load(PATH_MODEL_N_THETA, weights_only=True))
    except:
    ...
    # while epoch < epochs:
        ...
        if epoch % 100 == 0:
            ...
            try:
                torch.save(N_omega.state_dict(), PATH_MODEL_N_OMEGA)
            except:
                print(f"impossible de sauvegarder le fichier {PATH_MODEL_N_OMEGA}")
            try:
                torch.save(N_theta.state_dict(), PATH_MODEL_N_THETA)
            except:
